...
export const shellStyles = {
  ...
  headerActions: 'flex min-w-0 flex-1 items-start justify-end gap-2 md:flex-none max-[380px]:gap-1.5',
  ...
  shell: 'group/shell block min-h-dvh overflow-x-clip font-(family-name:--font-ui) text-(--ink) md:grid md:grid-cols-[248px_minmax(0,1fr)] md:data-[sidebar=collapsed]:grid-cols-[72px_minmax(0,1fr)]',
  sidebar: 'sticky top-0 z-2 hidden h-dvh min-w-0 md:block',
  sidebarSurface: 'pointer-events-none absolute inset-y-0 left-0 w-[248px] origin-left bg-(--surface-soft) shadow-[inset_-1px_0_var(--hairline)] group-data-[sidebar=collapsed]/shell:scale-x-[0.29032258] group-data-[sidebar-motion=pointer]/shell:transition-transform group-data-[sidebar-motion=pointer]/shell:duration-(--duration-drawer) group-data-[sidebar-motion=pointer]/shell:ease-(--ease-drawer) motion-reduce:transition-none',
  ...
  navigation: 'grid gap-2',
  navigationLink: `flex min-h-12 items-center gap-2.5 rounded-xl p-3 text-sm leading-[1.4] font-medium text-(--body) no-underline aria-[current=page]:bg-[#eadbd0] aria-[current=page]:text-[#88492f] aria-[current=page]:shadow-[inset_0_0_0_1px_#dcc5b6] aria-[current=page]:[&_svg]:text-(--link) not-aria-[current=page]:hover:bg-(--surface-card) group-data-[sidebar=collapsed]/shell:justify-center [&_svg]:shrink-0 motion-safe:[&_svg]:transition-transform motion-safe:[&_svg]:duration-(--duration-enter) motion-safe:hover:not-focus-visible:[&_svg]:translate-x-0.5 ${feedback} ${focus}`,
  sidebarCreate: `mt-5 flex min-h-11 items-center gap-2.5 rounded-xl border border-(--hairline) bg-(--canvas) px-3 py-2.5 text-sm text-(--link) no-underline hover:bg-(--surface-card) group-data-[sidebar=collapsed]/shell:justify-center [&_svg]:shrink-0 focus-visible:outline-offset-[-2px] ${focus}`,
  ...
  workspace: 'min-w-0 bg-(--canvas)',
  topbar: 'flex min-h-19 bg-(--canvas) items-start justify-between gap-3 border-b border-(--hairline) px-5 py-3.5 md:min-h-[81px] md:items-center md:gap-4 md:px-6 md:py-4 lg:px-8 max-[380px]:px-4',
  ...
  desktopContext: 'hidden flex-wrap items-center gap-2.5 text-[0.8125rem] text-(--muted) lg:flex [&_svg]:size-3 [&_[aria-current]]:text-(--ink) [&_a]:inline-flex [&_a]:min-h-11 [&_a]:items-center [&_a]:text-(--body) [&_a]:underline [&_a]:decoration-(--hairline)',
  mobileBrand: 'inline-flex shrink-0 md:hidden',
  main: 'mx-auto max-w-7xl px-5 pt-7 pb-12 md:px-6 md:pt-10 md:pb-16 lg:px-10 min-[1600px]:px-12 max-[380px]:px-4',
  pageHeader: 'mb-6 border-b border-(--hairline) pb-6 md:mb-8 [&_h1]:mt-0 [&_h1]:mb-2 [&_h1]:font-(family-name:--font-display) [&_h1]:text-[2.25rem] [&_h1]:font-medium [&_h1]:tracking-[-0.025em] [&_p]:m-0 [&_p]:max-w-[70ch] [&_p]:text-(--muted)',
  ...
  drawer: 'fixed top-0 left-0 z-50 h-dvh max-h-dvh w-[min(320px,calc(100%_-_32px))] overflow-y-auto overscroll-contain rounded-r-(--radius-lg) border-r border-(--hairline) bg-(--surface-soft) p-6 font-(family-name:--font-ui) text-(--ink) shadow-[12px_0_40px_rgb(20_20_19/10%)] motion-reduce:transform-none',
  tooltip: 'z-100 max-w-60 rounded-lg bg-(--ink) px-3 py-2 font-(family-name:--font-ui) text-xs text-(--canvas) shadow-lg',
  dialog: 'outline-none',
