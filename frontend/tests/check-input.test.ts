...
test('raw source and submission preserve duplicate keys for server rejection', () => {
  const source = '{"brand":"first", "brand":"ProofPay"}';
  const submission = '{"brand":"one", "brand":"two"}';
  const body = buildRawRequest({ source, submission, preservePlaceholders: true, requiredTerms: 'ProofPay, invoice' });
  assert.ok(body.includes('"source":' + source));
  assert.ok(body.includes('"submission":' + submission));
  assert.deepEqual(JSON.parse(body).rules, { preserve_placeholders: true, required_terms: ['ProofPay', 'invoice'] });
});
...
test('request validates both documents and safely escapes terms', () => {
  ...
  const body = buildRawRequest({ source: '{}', submission: '{}', preservePlaceholders: false, requiredTerms: 'a"b,  ,ProofPay' });
  assert.deepEqual(JSON.parse(body).rules.required_terms, ['a"b', 'ProofPay']);
});
