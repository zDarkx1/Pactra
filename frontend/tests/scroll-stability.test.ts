...
test('creative scenes keep semantic readable server content and separate visual layers',()=>{const p=read('../components/scope-story.tsx');assert.match(p,/data-scope-paper/);assert.match(p,/data-scope-seal/);assert.match(p,/id="capabilities"/);assert.match(p,/No funds move/)});
