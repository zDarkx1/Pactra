...
- **Dispute arbiter:** A real human selected by the Pactra team, not an AI agent. The designated arbiter must be disclosed to and accepted by both buyer and worker before funding. Authority is limited to allocating the disputed deliverable's funds between the parties, with no payment to the arbiter or access to unrelated allocations. Owner confirmed a team-selected human arbiter; implementation pending. Decision deadline, inactivity fallback, conflicts of interest, replacement and any future fee policy remain unresolved.
