...
## Start safely
Bind HOST=127.0.0.1 unless an authenticated, reviewed proxy is configured. Keep DATABASE_URL on the Go process. No secrets in CLI arguments, shell history or public logs. Provisioned ProofPay runtime role has no schema-creation privileges; migrations must be explicit, separate administrative operations.
...
Before real customer data, use pg_dump restricted to schema proofpay through a secure admin/migration connection. Pass credentials via a mode0600 pgpass file or private process environment, never command-line URI. Encrypt the resulting backup and restrict access: manifests can contain private source text and session/challenge records are security-sensitive. Restore ONLY to an isolated test database, never blindly over hosted data. Verify constraints and participant access tests there. This document describes a required drill; it does not claim it has been performed.
